...
public class OkGoUtils {
    ...
    public static void initOkGo(Application application) {
        ...
        builder.cookieJar(new CookieJarImpl(new SPCookieStore(application)));            //使用sp保持cookie，如果cookie
        // 不过期，则一直有效
        //        builder.cookieJar(new CookieJarImpl(new DBCookieStore(application)));
        // 使用数据库保持cookie，如果cookie
        // 不过期，则一直有效
        ...
        //HttpsUtils.SSLParams sslParams4 = HttpsUtils.getSslSocketFactory(getAssets().open("xxx.bks"), "123456",
        // getAssets().open("yyy.cer"));
